...
var Recorder = /*#__PURE__*/function () {
  function Recorder(source, cfg) {
    ...
    this.worker = new _inlineWorker["default"](function () {
      ...
      function exportSecondsToWAV(type) {
        var seconds = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
        var samplesPerSecond = Math.ceil(seconds / (bufferLen / sampleRate));
        var buffers = [];

        for (var channel = 0; channel < numChannels; channel++) {
          var recBuffersArr = recBuffers[channel];
          var newRecBuffersArr = [];

          for (var i = samplesPerSecond; i > 0; i--) {
            if (recBuffersArr[recBuffersArr.length - i]) {
              newRecBuffersArr.push(recBuffersArr[recBuffersArr.length - i]);
            }
          }

          var _recLength = newRecBuffersArr.length * bufferLen;

          buffers.push(mergeBuffers(newRecBuffersArr, _recLength));
        }

        var audioBlob = buffersToBlob(buffers, type);
        this.postMessage({
  ...
  _createClass(Recorder, [{
    ...
    key: "exportSecondsToWAV",
    value: function exportSecondsToWAV(cb, seconds, mimeType) {
      mimeType = mimeType || this.config.mimeType;
      cb = cb || this.config.callback;
      if (!cb) throw new Error('Callback not set');
